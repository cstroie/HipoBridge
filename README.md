...
The MWL server starts automatically alongside the HTTP server when `worklist.cfg` is present. Copy `examples/worklist.cfg.example` to get started. See [docs/WORKLIST.md](docs/WORKLIST.md) for full documentation.

```bash
pip install pynetdicom pydicom
cp examples/worklist.cfg.example worklist.cfg   # fill in credentials and device sections
```
