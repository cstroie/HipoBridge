...
- File logging is off by default; set `[logging] file` in `local.cfg` (or `--log-file`) to enable the size-limited `RotatingFileHandler` (`max_bytes`/`backup_count`, default 10MB × 5). Wired in `init_app()`.
...
- **Cancel**: same class, same form-replay mechanism (`_replay_form()`), but POSTs with `hdnAction=A` and no field overrides — mirrors Hipocrate's own "Anulează" button (captured from a live browser POST body). No signal comes back from a fresh `cerere.asp` fetch that a request was cancelled, so the frontend tracks it as a local flag (`article.dataset.cancelled` / `modal.dataset.cancelled`) set right after a successful call — see the frontend gotcha below.
