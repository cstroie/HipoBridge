...
- Prompts live in `llm/prompts/<kind>.md`, not hardcoded in `prompts.py` — edit the `.md` file to tune a prompt, no code change needed. `llm/prompts/system.md` holds the fixed, non-task-specific rules shared by every kind (role framing, output language via `{language}` placeholder, anti-fabrication/no-preamble backstop) — every call's system message is this file + an optional date directive + the kind's task template, concatenated into one system message (not separate system-role entries — local chat templates commonly only render one).
