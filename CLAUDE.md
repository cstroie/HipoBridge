...
- `requested_by` is gone from the schedule row for good, but `cerere.asp` still exposes the ordering physician (`HippoClientCerere` → `request.physician`, from the `strMedicId` select). `worklist.py`'s DICOM MWL path already fetches `cerere.asp` per request during `_enrich()` (for exams/justification/section) — `physician` was added to that same enrichment dict and `_build_datasets`' `referrer` now prefers it over the empty schedule-row field, so `ReferringPhysicianName` in the DICOM worklist is unaffected. Deliberately **not** wired into `/api/schedule`/`/fhir/Schedule` itself — that would mean an extra `cerere.asp` fetch per listed row (up to ~100-200 on an unfiltered day) under the shared semaphore(6), which was judged not worth the listing slowdown.
- The frontend timeline row's requester slot (`.timeline-meta-requester`) is empty at build time (schedule bundle has none) but kept in the DOM with `hidden = true` rather than removed, with a reference stashed on `regionLine._requesterEl`/`_requesterSep`. `scheduleExamObserver` (the lazy per-row fetch triggered when a row scrolls into view) reveals it via `_applyReferrer()` once the row's single `?type=solicitare` fetch resolves (see below) — **`ImagingStudy` has no `referrer` field, only `performer`** (who did the exam, not who ordered it), so region/indication/referrer all now come from the same one request instead of `ImagingStudy` for the former two.
- **2026-07-28, superseded the above two points**: the schedule row's lazy fetch is now a single request to `BuletinSolicitare.asp` (`HippoClientBuletinSolicitare`, routed via `/fhir/ServiceRequest/{id}?type=solicitare`) instead of the `cerere`/`ImagingStudy`/fallback-`ServiceRequest` combo. That page (the request/order form, title "FISA DE SOLICITARE") gives region (`Organ tinta / segment anatomic`), indication (`Justificare`), and — the reason for the switch — **the correct ordering physician**. `cerere.asp`'s `strMedicId` (what `request.physician` was reading) only ever resolves to **Medic curant** (attending physician), not **Medic solicitant** (who actually placed this specific order) — the two differ whenever the patient's regular doctor isn't the one who ordered the exam. `HippoClientBuletinSolicitare.fhir_response` sets `requester` from Medic solicitant, falling back to Medic curant only if no distinct orderer was recorded. `type=63&IdP=70` are fixed and work across modalities (verified on eco/radio/ct) despite the page always being titled "FISA SOLICITARE ECOGRAFIE". Per-row upstream cost is now exactly 1 request, down from 2–4.
- `BuletinAnalize.asp?type=2` was also inspected as a candidate and rejected: its `MEDIC:` header field is Medic curant too (not the orderer), and the actual performer only appears as `Validat de:` once a report is finalized — not useful for an unperformed row, and not needed at all for what the schedule row displays (region/indication/referrer, not performer).
