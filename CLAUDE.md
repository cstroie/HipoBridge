...
- The frontend timeline row's requester slot (`.timeline-meta-requester`) is empty at build time (schedule bundle has none) but kept in the DOM with `hidden = true` rather than removed, with a reference stashed on `regionLine._requesterEl`/`_requesterSep`. `scheduleExamObserver` (the lazy per-row fetch triggered when a row scrolls into view) fetches `ServiceRequest?type=cerere` unconditionally for the referrer and reveals the slot via `_applyReferrer()` — **`ImagingStudy` has no `referrer` field, only `performer`** (who did the exam, not who ordered it), so the referrer fetch cannot be folded into the "study already reported" fast path the way region/indication are; it always needs its own `cerere` call.
- Per-row upstream cost once visible: 2 requests (`cerere` + `ImagingStudy`) if already reported, 3 if not yet reported (adds a plain `ServiceRequest` fetch for orderDetail). `cererePromise` is created once and reused by both the referrer handler and the not-yet-reported fallback (which also needs Justificare text) — don't reintroduce a second `?type=cerere` fetch for the same row.
