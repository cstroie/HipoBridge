...
- **Known limitation**: `_build_datasets`' `ReferringPhysicianName` (`referrer` in `_build_datasets`) is sourced from `cerere.asp` via `_enrich()`'s `physician` field — Medic curant (attending), not Medic solicitant (see the Medic curant vs. Medic solicitant note under **Schedule**). The DICOM worklist's referring-physician field is therefore subject to the same inaccuracy the schedule row used to have, just not yet fixed here — doing so would mean adding a `BuletinSolicitare.asp` fetch to `_enrich()` alongside the existing `cerere`/patient/checkin fetches per request.
