...
- The frontend timeline row's requester slot (`.timeline-meta-requester`) is empty at build time (schedule bundle has none) but kept in the DOM with `hidden = true` rather than removed, with a reference stashed on `regionLine._requesterEl`/`_requesterSep`. `scheduleExamObserver` (the same lazy per-row fetch that already loads `ImagingStudy`/`ServiceRequest?type=cerere` for the region/indication line when the row scrolls into view) also reads `referrer?.display` off those same responses and reveals the slot via `_applyReferrer()` — no extra fetch needed since these requests were already being made.
