...
**Outcome:** the condition or outcome at discharge.
...
- If the source does not mention something for a line, write [not available] after that label rather than filling the gap with typical, expected, or 'textbook' findings.
...
- Never contradict the source (e.g. if the patient is afebrile, do not write febrile).
...
- Keep every line even if its content is [not available]. Output ONLY the four lines: no preamble, no reasoning or thinking steps.
